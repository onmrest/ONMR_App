...
import 'package:jamrekha/i18n/i18n.dart';
import 'package:jamrekha/src/elements/DrawerMenu.dart';
import 'package:jamrekha/src/elements/SocialItemMenu.dart';
import 'package:jamrekha/src/elements/WebViewElementState.dart';
import 'package:jamrekha/src/helpers/HexColor.dart';
import 'package:jamrekha/src/models/menu.dart';
import 'package:jamrekha/src/models/setting.dart';
import 'package:jamrekha/src/models/settings.dart';
import 'package:jamrekha/src/models/social.dart';
import 'package:jamrekha/src/pages/AboutScreen.dart';
import 'package:jamrekha/src/pages/LanguageScreen.dart';
import 'package:jamrekha/src/pages/NoticiationScreen.dart';
                  ...
                  children: <Widget>[
                    SizedBox(
                      width: double.tryParse(
                          Setting.getValue(widget.settings.setting!, "logo_drawer_width"))?? 70,
                      height: double.tryParse(
                          Setting.getValue(widget.settings.setting!, "logo_drawer_height"))?? 70,
                      child: Image.network(Setting.getValue(
                          widget.settings.setting!, "logo_drawer")),
                    ),
                    Padding(
            ...
            (Social social) => SocialItemMenu(
                ...
                onTap: () async {
                  if (await canLaunchUrl(
                      Uri.parse(social.linkUrl!.replaceAll("id_app", social.idApp!)))) {
                    await launchUrl(
                        Uri.parse(social.linkUrl!.replaceAll("id_app", social.idApp!)));
                  } else {
                    launchUrl( Uri.parse(social.url!.replaceAll("id_app", social.idApp!)));
                  }
                }),
          )
          .toList(),
